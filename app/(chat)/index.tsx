...
export const WS_BASE = __DEV__
  ...
  : process.env.EXPO_PUBLIC_WS_BASE?.trim() ||
    "wss://api.haniumpicky.click/wss/chat"; // 운영
...
export default function ChatScreen() {
  ...
  const amIBuyer =
    myUserId != null && sellerId != null && myUserId !== sellerId;
  ...
  const fetchTradeStatus = useCallback(async () => {
    ...
    try {
      const {
        ...
        status,
        productId: pid,
      } = await getTradeStatus(roomId, wsToken);
  ...
  const toUi = useCallback(
    (m: ChatMessageDTO): Message => {
      ...
      if (normalizedType === "SYSTEM") {
        ...
        const ctaVisible =
          receiverId != null ? Number(receiverId) === myUserId : isSeller;
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      console.log("[WS URL]", url, {
        protocol: Platform.OS === "web" ? window.location.protocol : "native",
      });
      ...
      ws.onmessage = (ev) => {
        try {
          ...
          if (
            raw?.type === "DIRECT_REQUEST" ||
            raw?.type === "PARCEL_REQUEST"
          ) {
  ...
  const handleRequestMeetup = useCallback(async () => {
    ...
    try {
      const { sellerId: sellerIdFromServer, message } =
        await requestDirectTrade(roomId, wsToken);
  ...
  const handleRequestParcel = useCallback(async () => {
    ...
    try {
      const { sellerId: sellerIdFromServer, message } =
        await requestParcelTrade(roomId, wsToken);
  ...
  const acceptDirect = useCallback(async () => {
    if (!roomId || !wsToken) return;
    try {
      const { buyerId: buyerIdFromServer, message } = await acceptDirectTrade(
        roomId,
        wsToken
      );
  ...
  const goToReview = useCallback(() => {
    if (!tradeId) {
      Alert.alert(
        "평가",
        "거래 ID를 찾을 수 없어요. 잠시 후 다시 시도해주세요."
      );
      return;
    }
    router.push({
